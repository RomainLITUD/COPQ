...
def get_multi_rewards_old(env_name, nstate):
  if env_name in ['hopper', 'walker2d']:
    multi_reward = jnp.array([-nstate.metrics['reward_forward'],
                              nstate.metrics['reward'] # - 0.01*nstate.metrics['reward_ctrl']
                              ])

  elif env_name in ['ant']:
    multi_reward = jnp.array([-nstate.metrics['velocity_total'],
                              nstate.metrics['reward'] # - 0.5*nstate.metrics['reward_ctrl']
                              ])
  else:
    multi_reward = jnp.array([-nstate.metrics['reward_linvel'],
                              nstate.metrics['reward'] # - 0.1*nstate.metrics['reward_quadctrl']
                              ])

  return multi_reward

def get_multi_rewards(env_name, nstate):
  if env_name in ['hopper']:
    multi_reward = jnp.array([nstate.metrics['reward_healthy'], nstate.metrics['reward_forward'] - nstate.metrics['reward_ctrl']
                              ])
    
  elif env_name in ['walker2d']:
    multi_reward = jnp.array([nstate.metrics['reward_healthy'], nstate.metrics['reward_forward'] - 0.5*nstate.metrics['reward_ctrl']
                              ])

  elif env_name in ['ant']:
    multi_reward = jnp.array([nstate.metrics['reward_survive'], nstate.metrics['reward_forward'] - nstate.metrics['reward_ctrl']
                              ])
  else:
    multi_reward = jnp.array([nstate.metrics['reward_alive'], 1.25*nstate.metrics['forward_reward'] - nstate.metrics['reward_quadctrl']
                              ])

  return multi_reward

def get_multi_rewards_easy(env_name, nstate):
  if env_name in ['hopper']:
    multi_reward = jnp.array([-nstate.metrics['reward_ctrl'], nstate.metrics['reward_healthy'] + nstate.metrics['reward_forward']
                              ])
    
  elif env_name in ['walker2d']:
    multi_reward = jnp.array([-0.5*nstate.metrics['reward_ctrl'], nstate.metrics['reward_healthy'] + nstate.metrics['reward_forward']
                              ])

  elif env_name in ['ant']:
    multi_reward = jnp.array([-nstate.metrics['reward_ctrl'], nstate.metrics['reward_survive'] + nstate.metrics['reward_forward']
                              ])
  else:
    multi_reward = jnp.array([-nstate.metrics['reward_quadctrl'], nstate.metrics['reward_alive'] + 1.25*nstate.metrics['forward_reward']
                              ])
