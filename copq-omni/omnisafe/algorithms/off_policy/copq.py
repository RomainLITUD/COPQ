...
class COPQ(SAC):
    ...
    def _update_critic(
        ...
        with torch.no_grad():
            ...
            lam = self._lagrange.lagrangian_multiplier.data.item()
            # lam = 0.025
            v1 = lam/((lam**2+1.)**0.5)
            v2 = 1./((lam**2+1.)**0.5)

            # q_cost = -next_q[:, 0].mean(-1) + next_q[:, 0].std(-1)/3

            # rect = torch.clamp(self.convex * (self._lagrange.cost_limit/10 - q_cost.mean()), max=lam)
            # coeff = (lam-rect).detach()

            # vc1 = coeff/((coeff**2+1.)**0.5)
            # vc2 = 1./((coeff**2+1.)**0.5)


            cholesky_term = torch.stack([l11*v1, (l21*v1+l22*v2)], -1)*0.5
    ...
    def _update_actor(  # pylint: disable=too-many-arguments
        ...
        loss_pi, cost_pred, coeff = self._loss_pi(obs, act)
        ...
        self._logger.store(
            {
                'Metrics/LagrangeMultiplier': self._lagrange.lagrangian_multiplier.data.item(), #coeff.data.item(), #
            },
        )
    ...
    def _loss_pi(
        ...
        with torch.no_grad():
            ...
            q_current_cost = -q_current[:, 0].mean(-1) + q_current[:, 0].std(-1)/3.
        ...
        q_reward = next_q[:,1].mean(-1) - (l21*v1 + l22*v2)*0.5 #/(2**0.5)
        q_cost = -next_q[:,0].mean(-1) + l11*v1*0.5 #/(2**0.5)
        actor_loss = self._alpha * log_prob - q_reward + coeff*q_cost
        # actor_loss = self._alpha * log_prob - q_reward + lam*q_cost
        return actor_loss.mean(), q_current_cost.mean(), coeff.mean() #-q_current_cost_raw.mean()
