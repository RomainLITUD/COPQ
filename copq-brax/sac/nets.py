...
class QModule(linen.Module):
  ...
  def __call__(self, obs: jnp.ndarray, actions: jnp.ndarray):
    ...
    if self.mode == 'mo':
      q = MLP(
          layer_sizes=list(self.hidden_layer_sizes) + [self.num_obj],
          activation=self.activation,
      ...
      q = MLP(
          layer_sizes=list(self.hidden_layer_sizes) + [5],
          activation=self.activation,
          kernel_init=jax.nn.initializers.lecun_uniform())(
              hidden)
      L11 = jax.nn.softplus(q[...,2:3]) + 0.1
      L22 = jax.nn.softplus(q[...,3:4]) + 0.1
      L21_free = jnp.tanh(q[...,-1:])
      A = 9 * (L11 / L22)
      t_max = jnp.clip((A - 1.0) / jnp.sqrt(jnp.clip(2.0*A - 1.0, min=1e-3)), min=0.0)
      L21 = (0.95* t_max) * L22 * L21_free
      out = jnp.concatenate([q[...,:2], L11, L22, L21], -1) 
      return out
    else:
      q = MLP(
