...
def fixed_count_mask(key, batch_size, num_pairs):
    half = batch_size // 2
    keys = jax.random.split(key, num_pairs)
    def col_fn(k):
        idx = jax.random.choice(k, batch_size, shape=(half,), replace=False)
        col = jnp.zeros(batch_size, dtype=jnp.int32)
        return col.at[idx].set(1)
    mask = jax.vmap(col_fn, in_axes=0, out_axes=1)(keys)
    return mask
...
def train(seed,
  ...
  def sgd_step(
    ...
    cond_fn = lambda _: jax.tree_util.tree_map(
        lambda x, y: x * (1 - tau) + y * tau,
        training_state.target_q_params, q_params)

    else_fn = lambda _: training_state.target_q_params

    new_target_q_params = jax.lax.cond(
        training_state.gradient_steps % target_delay == 0,
        cond_fn,
        else_fn,
        operand=None  # No extra input needed for the branches
    )
  ...
  def get_eval_performance_crl(metrics):
    if name in ['hopper', 'walker2d']:
        y1 = metrics['eval/episode_reward_forward']

    elif name in ['ant']:
        y1 = metrics['eval/episode_velocity_total']

    elif name in ['humanoid']:
        y1 = metrics['eval/episode_reward_linvel']

    y2 = metrics['eval/episode_reward']
    return [y1, y2]
  
  def get_eval_performance_lrl(metrics):
    if name in ['hopper']:
        y1 = metrics['eval/episode_reward_healthy'] #+ metrics['eval/episode_reward_forward']
        y2 = metrics['eval/episode_reward_forward'] - metrics['eval/episode_reward_ctrl']

    elif name in ['walker2d']:
        y1 = metrics['eval/episode_reward_healthy'] #+ metrics['eval/episode_reward_forward']
        y2 = metrics['eval/episode_reward_forward'] - 0.5*metrics['eval/episode_reward_ctrl']

    elif name in ['ant']:
        y1 = metrics['eval/episode_reward_survive'] # + metrics['eval/episode_reward_forward']
        y2 = metrics['eval/episode_reward_forward'] - metrics['eval/episode_reward_ctrl']

    elif name in ['humanoid']:
        y1 = metrics['eval/episode_reward_alive'] # + metrics['eval/episode_forward_reward']
        y2 = 1.25*metrics['eval/episode_forward_reward'] - metrics['eval/episode_reward_quadctrl']
  ...
  if process_id == 0 and num_evals > 1:
    ...
    xdata.append(0)
    if task_name == 'crl':
        y_ = get_eval_performance_crl(metrics)
    if task_name == 'lrl':
        y_ = get_eval_performance_lrl(metrics)
    ydata.append(y_)
